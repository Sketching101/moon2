...
namespace BzKovSoft.ObjectSlicer
{
	public class BzMeshDataEditor
	{
		...
		public int GetIndexFor(int from, int to)
		{
			...
			if (Mathf.Abs((enter - v1).magnitude) <= 0.0001f)
				return from;

			if (Mathf.Abs((enter - v2).magnitude) <= 0.0001f)
				return to;
		...
		public List<PolyMeshData> CapSlice(Material capMaterial)
		{
			...
			{
				var loop = new BzPolyLoop(_meshData, edgeLoop, _planeLocal.normal, _adapter);
				if (!loop.Created)
					continue;
		...
		internal void EdgeLoops_JoinBySameValue(LinkedList<LinkedLoop<int>> edgeLoops)
		{
			...
			while (parrentListsNode != null)
			{
				...
				while (joinCandidate != null)
				{
					...
					{
						joinCandidate.Value.first.Remove();// to avoid duplicate of the same index when concatinaiting
		...
		float CalcGetRatio(Vector3 v1, Vector3 v2, out Vector3 enter)
		{
			...
			float enterD;
			_planeLocal.Raycast(ray, out enterD);
